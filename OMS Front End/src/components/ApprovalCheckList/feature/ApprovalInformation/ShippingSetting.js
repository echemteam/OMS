...
const ShippingSetting = ({ isModelOpen, mainId, approvalChekedData, handleCheckbox }) => {
    ...
    useEffect(() => {
        if (!isGetDefaultValueFetching && isGetDefaultValueSuccess && isGetDefaultValueData) {
            ...
            setDeliveryMethodsList(isGetDefaultValueData.deliveryMethodsList.filter(data => data.isPrimary));
            isGetDefaultValueData.shppingDeliveryCarriersList && setDeliveryCarriersList(isGetDefaultValueData.shppingDeliveryCarriersList.filter(data => data.isPrimary));
        }
                ...
                {shippingInformation && (
                    <div className="card-part">
                        <h6 className="name-title">
                            ...
                            <p className="name-desc" style={{ color: 'var(--themeColor)', fontSize: 'var(--commonFont)', fontWeight: '700' }}>
                                {shippingInformation.name}
                            </p>
                        </h6>
                        {(deliveryMethodsList.length > 0) && <h6 className="title mt-4 mb-0">Delivery Method Details: </h6>}
                        {(deliveryMethodsList.length > 0) && deliveryMethodsList.map((data, index) => (
                            <>
                                {/* This div used for the line */}
                                <div className="card-part" style={{ paddingBottom: '1px' }} key={index}></div>
                                <h6 className="name-title">
                                    <span className="label">Zone:</span>
                                    <p className="name-desc">{data.isForInternational ? 'International' : 'Domestic'}</p>
                                </h6>
                                <h6 className="name-title">
                        ...
                        {(deliveryCarriersList.length > 0) && <h6 className="title mt-4 mb-0">Carrier Details: </h6>}
                        {(deliveryCarriersList.length > 0) && deliveryCarriersList.map((data, index) => (
                            <>
                                {/* This div used for the line */}
                                <div className="card-part" style={{ paddingBottom: '1px' }} key={index}></div>
                                <h6 className="name-title">

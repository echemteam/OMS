...
const AddEditAddress = forwardRef(
  ({
    addressContactType,
    ...
    const [getAllStates, { data: allGetAllStatesData }] =
      useLazyGetAllStatesQuery();
    const [
      getAllCountries,
      { isSuccess: isGetAllCountriesSucess, data: allGetAllCountriesData },
    ...
    useEffect(() => {
      if (isOrderManage) {
        ...
        if (getAddressTypeIdOrder === AddressType.BILLING) {
          form = removeFormFields(addressFormData, [
            "isShippingAndBilling",
            "isPreferredShipping",
          ]);
    ...
    const setInitialValue = () => {
      if (!editMode) {
        let updatedFormData;
        updatedFormData = removeFormFields(addressFormData, [
          "isPreferredShipping",
          "isShippingAndBilling",
          "isPreferredBilling",
        ]);
    ...
    useEffect(() => {
      if (isGetByIdData) {
        ...
        if (isGetByIdData.type === "Billing") {
          ...
          isGetByIdData.type === "AP" ||
          isGetByIdData.type === "Primary"
        ) {
          updatedFormData = removeFormFields(addressFormData, [
            "isShippingAndBilling",
            "isPreferredBilling",
    ...
    const buildTransformedData = (data, isSupplier, keyId, editMode) => {
      ...
      const { id: stateId, name: stateName } = transformLocationData(
        data.stateId,
        data.stateName
      );
      const { id: cityId, name: cityName } = transformLocationData(
        data.cityId,
        data.cityName
      );
    ...
    const getCustomerAddressId = (isSupplier, isGetByIdData) => {
      return isSupplier === false
        ? isGetByIdData
          ? isGetByIdData.customerAddressId
          : 0
        : 0;
    };
    ...
    const getSupplierAddressId = (isSupplier, isGetByIdData) => {
      return isSupplier === true
        ? isGetByIdData
          ? isGetByIdData.supplierAddressId
          : 0
        : 0;
    };
    ...
    const buildUpdateData = (transformedData, isGetByIdData, isSupplier) => {
      ...
      return {
        ...transformedData,
        addressId:
          isGetByIdData === null ? addressEditTableId : isGetByIdData.addressId,
        customerAddressId,
        supplierAddressId,
      };
    };
    ...
    const handleColumnChange = (dataField, updatedData) => {
      ...
      if (dataField === "countryId") {
        ...
        const billingEditFields = [
          "isPreferredShipping",
          "isShippingAndBilling",
        ];
        const shippingFields = ["isPreferredBilling"];
        const shippingEditFields = [
          "isPreferredBilling",
          "isShippingAndBilling",
        ];
        ...
        switch (addressTypeId) {
          case AddressType.BILLING:
            manageData = editMode
              ? removeFormFields(addressFormData, billingEditFields)
    ...
    useEffect(() => {
      let data = { ...formData };
      if (
        selectedCheckboxFeild === "isShippingAndBilling" &&
        selectedCheckbox === false &&
        data.initialState.addressTypeId === 1
      ) {
        let updatedFormData;
        ...
        selectedCheckboxFeild === "isShippingAndBilling" &&
        selectedCheckbox === false &&
        data.initialState.addressTypeId === 2
      ) {
        let updatedFormData;

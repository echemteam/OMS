...
//** Component's */
const EmailAddressGrid = React.lazy(() =>
  import("../../EmailAddress/EmailAddressGrid")
);
const ContactNumbersGrid = React.lazy(() =>
  import("../../ContactNumber/ContactNumbersGrid")
);

const AddEditContact = forwardRef(
  ({
    contactTypeId,
    keyId,
    isOrderContact,
      ...
      return {
        ...
        emailAddressList:
          emailAddressList.length > 0
            ? modifyEmailAddressData(emailAddressList)
            : null,
        phoneNumberList:
          phoneNumberList.length > 0
            ? modifyPhoneNumberData(phoneNumberList)
            : null,
        [isSupplier ? "supplierContactId" : "customerContactId"]: isSupplier
          ? supplierContactId
          : customerContactId,
      };
    };
    ...
    useEffect(() => {
      if (!isGetByIdFetching && isGetByIdSucess && isGetByIdData) {
        ...
        const sortedPhoneList = [...isGetByIdData.phoneNumberList].sort(
          (a, b) => a.phoneId - b.phoneId
        );
        ...
        const sortedEmailList = [...isGetByIdData.emailAddressList].sort(
          (a, b) => b.emailId - a.emailId
        );
          ...
          <React.Fragment>
            ...
            <div className="row">
              <EmailAddressGrid
                ...
                setEmailAddressList={setEmailAddressList}
                contactId={contactId}
                isOrderManage={isOrderManage}
              />
              <ContactNumbersGrid
                isButtonDisable={isButtonDisable}

...
namespace OMS.FileManger.Services
{
    public class FileManager
    {
        ...
        public static string SaveFile(string Base64FileData, string path, string FileName)
        {
        ...
        /// <summary>
        /// Delete all the file in given directory
        /// </summary>
        ...
        {
            EmptyFolder(new DirectoryInfo(strDirPath));
        ...
        /// </summary>
        /// <param name="strFilePathFrom">Target Path</param>
        /// <param name="strFilePathTo">Destination Path</param>
        /// <param name="OverWrite">True if want to overWrite</param>
        /// <param name="RemoveAllFiles">True if want to Remove all files</param>
        public static void CopyFile(string strFilePathFrom, string strFilePathTo, bool OverWrite, bool RemoveAllFiles)
        {
            if (RemoveAllFiles)
            {
                ClearDirectory(GetFilePath(strFilePathTo));
            }
            File.Copy(strFilePathFrom, strFilePathTo, OverWrite);
        }

        /// <summary>
        /// Copy File From one location to other location with custom file name
        /// </summary>
        /// <param name="strFilePathFrom">Target Path</param>
        /// <param name="strFilePathTo">Destination Path</param>
        /// <param name="strDestFileName">Destination File Name</param>
        /// <param name="OverWrite">True if want to overWrite</param>
        /// <param name="RemoveAllFiles">True if want to Remove all files</param>
        public static void CopyFile(string strSrcFilePath, string strDestFilePath, string strDestFileName, bool OverWrite, bool RemoveAllFiles)
        {
            ...
            File.Copy(strSrcFilePath, strDestFilePath + strDestFileName + GetExtension(strSrcFilePath), OverWrite);
        }
        ...
        /// <returns>Get Proper File Path</returns>
        public static string GetFilePath(string strFilePath)
        {
            ...
            if (!string.IsNullOrEmpty(strRetVal))
            {
                if (strFilePath.Contains(@"\") && strFilePath.Contains("/"))
                {
                    ...
                    {
                        return strFilePath.Substring(0, strFilePath.LastIndexOf("/") + 1);
                    }
                    return strFilePath.Substring(0, strFilePath.LastIndexOf(@"\") + 1);
                }
        ...
        /// </summary>
        /// <param name="strFile">File Name with Path</param>
        /// <returns>Get File Extention</returns>
        ...
        /// <summary>
        /// Extrect the file name from the given path
        /// </summary>
        /// <param name="strFilePath">file path</param>
        ...
        public static string GenrateFileName(string path, string fileName)
        {
            string tempFileName = RandomString(8) + '_' + fileName;
            string tempNewFileName = path + tempFileName;

            for (int i = 1; File.Exists(tempNewFileName); i++)
            {
                tempFileName = fileName.Insert(fileName.LastIndexOf('.'), "_" + i);
                tempNewFileName = path + fileName.Insert(fileName.LastIndexOf('.'), "_" + i);
            }
            return tempFileName;
        }
        public static string GetUniqueFilename(string path, string fileName)
        {
            ...
            string tempNewFileName = fileName;
            tempNewFileName = path + "\\" + tempNewFileName;
            for (int i = 1; File.Exists(tempNewFileName); i++)
            {
                tempFileName = fileName.Insert(fileName.LastIndexOf('.'), "(" + i + ")");
                tempNewFileName = path + fileName.Insert(fileName.LastIndexOf('.'), "(" + i + ")");
            }
            return tempFileName;
        }
        ...
        /// <summary>
        /// Build Mappings
        /// It is Dictionary, which is stored all mime type 
        /// </summary>
        /// <returns></returns>
